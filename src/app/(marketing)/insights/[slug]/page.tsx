...
export async function generateMetadata({
  ...
  return {
    title: post.title,
    description: post.excerpt || `${post.title} — from Carve insights.`,
    alternates: { canonical: `https://carve.ac/insights/${params.slug}` },
    openGraph: {
      title: post.title,
      description: post.excerpt || `${post.title} — from Carve insights.`,
      type: 'article',
      url: `https://carve.ac/insights/${params.slug}`,
...
export default async function PlgBlogPostPage({
  params,
}: {
  params: { slug: string };
}) {
  ...
  return (
    <>
      <ArticleJsonLd
        ...
        url={`https://carve.ac/insights/${post.slug.current}`}
        publishedTime={post.publishedAt}
        image={post.mainImage?.asset?.url ? urlFor(post.mainImage).width(1200).height(630).url() : undefined}
      />
